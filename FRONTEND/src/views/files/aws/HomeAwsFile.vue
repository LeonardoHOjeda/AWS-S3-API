  ...
  <div class="container">
    ...
    <div>
      <ModalComponent v-if="showModal" title="Eliminar archivo(s)" @close-modal="closeModal">
        <p class="text-xl m-0 p-0 text-white">¿Estás seguro de eliminar el(los) archivo(s) seleccionado(s)?</p>
    ...
    <template v-else>
      <div class="mx-4">
        ...
        >
          ...
          <Column field="Size" header="TAMAÑO" sortable>
            <template #body="{ data }"> {{ bytesToMegabytes(data.Size).toFixed(2) }} MB </template>
          </Column>
          <Column field="LastModified" header="FECHA" sortable>
...
import { bytesToMegabytes, formatDate } from '@/utils/helpers'
...
const loadFiles = async () => {
  try {
    files.value = await awsService.getFilesFromAws()
